...
import { ReferenceToEvidenceItem, UndevelopedArgument } from 'asac4ux-lib';
/**
 * jest test from evidence id
 * @param {string} id
 * @returns boolean
 */
function runTestOfEvidence(id) {
 ...
 */
function checkSupportedClaim(supportedClaim, indentNum) {
    ...
    if (supportedClaim.argument instanceof UndevelopedArgument) {
        ...
        const argument = supportedClaim.argument;
        ...
        if (infOrRef instanceof ReferenceToEvidenceItem) {
            ...
            const evidence = infOrRef.evidence;
            let result = false;
            try {
